...
-- Proba: Testare Haskell, 27.06.2025
...
data Deque a = Empty | Cons Pos a (Deque a) deriving Show
...
instance Eq a => Eq(Deque a) where
	Empty == Empty = True
	(Cons a1 d2) == (Cons a2 d2) = a1 == a2 && d1 == d2 
	_ == _ = False
	
toLists :: Deque a -> ([a], [a])
toLists = go ([], [])
	where
	go (ls, rs) Empty = (reverse ls, reverse rs)
	go (ls, rs) (Cons L x d) = go (x:ls, rs) d
	go (ls, rs) (Cons R x d) = go (ls, x:rs) d
	
instance Monoid(Deque a) where
	mempty = Empty
	mappend Empty d2 = d2
	mappend (Cons p x d1) d2 = Cons p x (mappend d1 d2)
		
	
instance Foldable Deque where
	foldr _ z Empty = z
	foldr f z (Cons _ x d) = f x (foldr f x d)
	
suntEventLtN :: Deque Int -> Int -> Int
suntEventLtN dq n = foldr (\x acc -> if even x && x < n then x + acc else acc) 0 allElems
	where
	(ls, rs) = toLists dq
	allElems =ls ++ rs
	
suntEventLtN' :: Deque Int -> Int -> Int
suntEventLtN' dq n = sum $ filter(\x -> even x && x < n) (ls ++ rs)
	where
	(ls, rs) = toLists dq
...
prop_suntEventLtN_equiv :: Deque Int -> Int -> Bool
prop_suntEventLtN_equiv dq n = suntEventLtN dq n == suntEventLtN' dq n

newtype WriterMaybe w a = WriteMaybe (runWriterMaybe :: (Maybe a,w))
deriving Show
...
instance Functor (WriterMaybe w) where
	fmap f (WriterMaybe(ma,w)) = WriterMaybe (fmap f ma, w)
	
instance Monoid w => Applicative (WriterMaybe w) where
	pure a = WriterMaybe (Just a, mempty)
	WriterMaybe(mf, w1) <*> WriterMaybe (ma, w2) =
		WriterMaybe (mf <*> ma, w1 `mappend` w2)
		

instance Monoid w => Monad (WriterMaybe w) where
WriterMaybe (Nothing, w1)  >>= _ = WriterMaybe(Nothing, w1_)
WriterMaybe (Just a, w1) >>= f=
	let WriterMaybe(mb, w2) = f a
	in WriterMaybe (mb, w1 `mappend` w2)
