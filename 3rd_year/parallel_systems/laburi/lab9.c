...
double analytic_laplacian(double x, double y){
    return -2*cos(x)*sin(y);
}

double numerical_laplacian(double x, double y){
    double second_p_derivative_x = (f(x + STEP,y) + f(x - STEP, y)- 2*f(x,y))/(STEP*STEP);
    double second_p_derivative_y = (f(x,y + STEP) + f(x, y-  STEP)- 2*f(x,y))/(STEP*STEP);  
    double laplacian = second_p_derivative_x + second_p_derivative_y ; 

    return laplacian;
}

void write_x_mesh(double matrix[N][N], double start_val){

    for(int i=0 ; i<N-1; i++){
        for(int j=0; j<N-1 ;j++){
            matrix[i][j] = start_val + j*STEP;
        }
    }
}

void write_y_mesh(double matrix[N][N], double start_val){
    for(int i=0 ; i<N-1; i++){
        for(int j=0; j<N-1 ;j++){
            matrix[i][j] = start_val+ i*STEP;
            }
        }
}

void write_z_mesh(double x_mesh[N][N], double y_mesh[N][N], double z_mesh[N][N], double start_val){
    for(int i=0 ; i<N-1; i++){
        for(int j=0; j<N-1 ;j++){
...
/**
 * Illustrates how to use the MPI_Cart_shift routine.
 * This code creates a cartesian topology, then retrieves the rank of
 ...
 **/
    ...
    char* neighbours_names[2] = {"left", "right"};
