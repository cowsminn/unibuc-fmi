...
void TestVectorFile()
{
	...
	// Avoid referencing Photon compile-time symbols that might not be available
	printf("Photon parameters: (build-time parameters may be unavailable in this compile unit)\n");
	
	// Use default filenames from header if available, otherwise fall back
#ifdef PHOTON_DEFAULT_IN_FILE
    char *in_file = PHOTON_DEFAULT_IN_FILE;
#else
    char *in_file = "in.txt";
#endif

#ifdef PHOTON_DEFAULT_OUT_FILE
    char *out_file = PHOTON_DEFAULT_OUT_FILE;
#else
	...
	printf("openfile");
       fp = fopen(in_file, "r");
       if (fp == NULL){
		   printf ("Error at open input file '%s'\n", in_file);
           exit(1);
	   }
	   ...
	   fprintf(f, "Photon hashing results (defaults used for filenames)\n");
